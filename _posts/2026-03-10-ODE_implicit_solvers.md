...
$$G(x_{n+1}) = \underbrace{(x_{n+1} - x_n)}_{\text{현실: 실제 변화량}} - \underbrace{h \cdot f(x_{n+1})}_{\text{이상: 물리 법칙이 요구한 변화량}} = 0$$

### '물리 법칙 위반 성적표'란 무엇인가?

이 잔차 식 $G$는 현재 우리가 추측한 $x_{n+1}$이 자연의 법칙을 얼마나 잘 따르고 있는지를 보여주는 **성적표**와 같다. 식의 의미를 공학적으로 뜯어보면 다음과 같다.
...
결국, Simscape가 시뮬레이션 중에 "수렴(Convergence)에 실패했다"는 메시지를 띄우는 것은, 이 성적표의 오차 $G$를 도저히 0으로 만들 수 없는 **물리적 모순**이 모델 안에 있다는 뜻이다.
...
### 업데이트 로직: $x_{new} = x_{old} - \frac{G}{J}$

이 식은 NR의 핵심 알고리즘으로, 다음과 같은 논리 구조를 가진다.

1. 위반 점수($G$)가 크면: "정답에서 멀리 떨어져 있구나!" → 더 많이 이동한다.
2. 기울기($J$)가 가파르면: "조금만 움직여도 점수가 팍팍 변하네?" → 신중하게 조금만 이동한다.
...
결국 고성능 시뮬레이션은 수학적 신뢰와 계산 효율성 사이의 줄타기이며, 그 중심에는 **Newton-Raphson**이라는 견고한 다리가 놓여 있다.
